...
public final class EngineRun {
    ...
    public func run(
        ...
        queue.async(group: group) {
            ...
            while let chunk = Self.readAvailable(fd) {
        ...
        queue.async(group: group) {
            var buffer = ""
            let fd = err.fileHandleForReading.fileDescriptor
            while let chunk = Self.readAvailable(fd) {
    ...
    static func readAvailable(_ fd: Int32) -> Data? {
        var buffer = [UInt8](repeating: 0, count: 4096)
        var n: Int
        repeat {
            n = buffer.withUnsafeMutableBytes {
                Darwin.read(fd, $0.baseAddress, 4096)
            }
