...
final class CropGeometryTests: XCTestCase {
    ...
    private let master = CropGeometry(
        sourceWidth: 2160, sourceHeight: 3840, aspectWidth: 4, aspectHeight: 5)
    ...
    func testTheWindowIsAlwaysEven() {
        ...
        for width in [1079, 1081, 2159, 3841] {
            let g = CropGeometry(
                sourceWidth: width, sourceHeight: 4000, aspectWidth: 4, aspectHeight: 5)
    ...
    func testALandscapeClipMovesTheWindowSideways() {
        let wide = CropGeometry(
            sourceWidth: 3840, sourceHeight: 2160, aspectWidth: 9, aspectHeight: 16)
        ...
        for (width, height) in [(2160, 3840), (1081, 3840), (3840, 2160), (3840, 2161)] {
            for deliverable in shapes {
                ...
                if last.stdout.isEmpty {
                    ...
                    XCTAssertEqual(
                        last.stdout, geometry.filter(at: geometry.maximumOffset),
                    ...
                    XCTAssertTrue(
                        past.stderr.contains("outside 0..\(geometry.maximumOffset)"),
    ...
    func testTheFirstCroppingTargetIsTheOneWithABox() {
        ...
        XCTAssertEqual(
            delivery.cropBoxTarget(portrait), .feed,
            "reels does not crop a 9:16 master, so it must not claim the box")
        XCTAssertEqual(
            delivery.cropBoxTarget(FrameSize(width: 3840, height: 2160)), .reels,
    ...
    func testTheCentreBoxIsWhereTheEngineCentres() throws {
        let engine = try engineCheckout()
        let cases = [
            (2160, 3840, 4, 5), (2160, 3842, 4, 5), (2160, 3841, 4, 5),
        ...
        for (width, height, aw, ah) in cases {
            ...
            XCTAssertEqual(
                ran.stdout, geometry.filter(at: geometry.centreOffset),
