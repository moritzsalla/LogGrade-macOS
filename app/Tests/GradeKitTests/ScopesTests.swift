...
final class ScopesTests: XCTestCase {
    ...
    func testTheRampFillsTheRangeAndNamesItsEnds() throws {
        let space = CGColorSpaceCreateDeviceRGB()
        let context = CGContext(
            data: nil, width: 256, height: 8, bitsPerComponent: 8,
            bytesPerRow: 0, space: space,
            bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue)!
        for x in 0..<256 {
            let v = Double(x) / 255
        ...
        XCTAssertGreaterThan(
            occupied, 180, "the ramp should cover most of the range, got \(occupied)")
        let quarters = stride(from: 0, to: 256, by: 64).map { start in
            scopes.luma[start..<min(256, start + 64)].reduce(0, +)
        }
        XCTAssertTrue(
            quarters.allSatisfy { $0 > 0 }, "a quarter of the range is empty: \(quarters)")
    }
    ...
    func testTheReferencesLandWhereTheirHuesShould() throws {
        ...
        func position(_ name: String) throws -> (x: Double, y: Double) {
            let reference = try XCTUnwrap(
                Scopes.references.first { $0.name == name },
        ...
        XCTAssertTrue(
            Scopes.references.allSatisfy { !$0.ral.isEmpty },
    ...
    func testSamplingIsDeterministicAndProportional() throws {
        ...
        XCTAssertEqual(
            dense.luma.firstIndex(where: { $0 > 0 }),
