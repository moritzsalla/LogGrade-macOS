...
final class EngineRunTests: XCTestCase {
    func testReadsEventsAndCodesFromBothStreams() throws {
        ...
        XCTAssertEqual(
            streamed, ["run_start", "clip_skipped", "run_done"],
    ...
    func testAFailureKeepsItsExitCodeAndItsReason() throws {
        ...
        XCTAssertTrue(
            outcome.stderrText.contains("REFUSING"),
    ...
    func testItRefusesToStartWhenThePreflightFails() throws {
        ...
        XCTAssertThrowsError(try EngineRun(engine: location).run(arguments: [])) { error in
            ...
            XCTAssertTrue(
                String(describing: error).contains("missing: \(location.gradeScript.path)"),
    ...
    func testTheChildGetsAPathThatFindsTheTools() throws {
        ...
        XCTAssertTrue(
            path.contains(engine.root.path),
