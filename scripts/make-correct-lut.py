#!/usr/bin/env python3
"""
Generate a 3D .cube LUT for the input-correction stage: exposure and white balance in linear, then
contrast and saturation in log.
...
midtone-weighted RGB trim. Writing the maths here instead keeps it in one place, and leaves the
render chain a `lut3d` like the conversion.
...
correction — which is what a colourist's log controls are, and the standard managed-colour order:
...
USAGE
    ./make-correct-lut.py OUT.cube [--exposure 0] [--temp 0] [--tint 0]
                                   [--contrast 1] [--saturation 1] [--size 33]
...
second, which is inside a slider's debounce.
"""
import argparse
import os
import sys
...
# Rec.709 luma weights. Saturation needs a luma, and this cube's output is fed to Apple's
...
def fingerprint(a):
    ...
    return title(
        "Input correction (exposure=%s temp=%s tint=%s contrast=%s saturation=%s size=%d)"
        % (number(a.exposure), number(a.temp), number(a.tint),
           number(a.contrast), number(a.saturation), a.size)
    )
